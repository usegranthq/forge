...
func (vq *VerificationQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (vq *VerificationQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (vq *VerificationQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (vq *VerificationQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (vq *VerificationQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (vq *VerificationQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := vq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
//		GroupBy(verification.FieldAttemptID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Verification.Query().
//		Select(verification.FieldAttemptID).
//		Scan(ctx, &v)
...
func (vq *VerificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Verification, error) {
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, verification.ForeignKeys...)
	}
...
func (vq *VerificationQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Verification, init func(*Verification), assign func(*Verification, *User)) error {
	...
	nodeids := make(map[uuid.UUID][]*Verification)
	for i := range nodes {
		if nodes[i].user_verifications == nil {
			continue
		}
		fk := *nodes[i].user_verifications
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_verifications" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
