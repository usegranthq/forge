...
func (uq *UserQuery) loadUserSessions(ctx context.Context, query *UserSessionQuery, nodes []*User, init func(*User), assign func(*User, *UserSession)) error {
	...
	for _, n := range neighbors {
		fk := n.user_user_sessions
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_user_sessions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_user_sessions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
